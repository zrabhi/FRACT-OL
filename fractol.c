...
int main(void)
{
    ...
    while(data.x < WIDTH)
    {
        ...
        while(data.y < HIEGHT)
        {
            //  data.c_r = 1.5 *(data.x - WIDTH / 2) / (0.5 * zoom * WIDTH) + moveX;
            // data.c_i = (data.y - HIEGHT / 2) / (0.5 *zoom * HIEGHT) + moveY; 
             ...
             if(data.iteration < data.max_iteration)
                  {
                    ...
                    // E83A14
                     //DA1212
            ft_my_put_pixel(&img, data.x, data.y, data.iteration * 0x603601);
                  }
        data.y++;
        }
     data.x++;
